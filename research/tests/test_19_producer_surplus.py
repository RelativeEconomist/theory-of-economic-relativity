...
Economic question
-----------------
In this configured market, is the value a seller's valuation assigns to
SELL equal to the market price minus reservation cost, so that producer
surplus can be read from it for sellers who sell?
...
The four sellers are independent agents in one single-period scenario with
no reality function. Producer surplus and its total are computed by the
test from each seller's own value_of(SELL); that arithmetic is
test-specific analysis, not a TER primitive, not R, and not a TER system
outcome. The test introduces no relationship between agent-level and
...
This is the mirror image of the buyer-side setup test_18 uses.
...
- Producer surplus is not a TER primitive, and no realized outcome or
  reality function is modeled. Under this specific valuation setup,
  agent.value_of(SELL) equals market price minus reservation cost, so for
  sellers who actually select SELL it can be read directly as producer
  surplus.
