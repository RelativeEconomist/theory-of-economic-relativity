...
Economic question
------------------
Can TER represent gains from trade where a buyer and seller both choose
to trade when the buyer's reservation value exceeds the seller's
reservation cost?
...
TER mapping
-----------
Core architecture, evaluated independently for each agent:

    (G, M, F̂, V, H, D) ──→ C

    G   maximize transaction value
    M   the quoted market price, assumed correctly observed by both
        agents
    F̂   BUY/DO_NOT_BUY (buyer), SELL/DO_NOT_SELL (seller) -- F equals
        F̂ for both
    V   ValuationRule.PRICE_TAKING -- buyer reservation value relative
        to price; seller price relative to reservation cost
    H   current transaction decision
    D   DecisionProcess.MAXIMIZE
    C   BUY or DO_NOT_BUY (buyer); SELL or DO_NOT_SELL (seller)
...
M     Model of reality       the quoted market price, assumed correctly
                              observed
F, F̂  Feasible sets          BUY/DO_NOT_BUY, SELL/DO_NOT_SELL; F̂
                              equals F for both agents
V     Valuation               reservation value relative to price
                              (buyer); price relative to reservation
                              cost (seller)
H     Time horizon           constant: current transaction decision
D     Decision process       constant: DecisionProcess.MAXIMIZE
C     Selected action        BUY (buyer), SELL (seller), observed result
...
this test, not TER primitives or realized O: they are read directly
from each agent's own agent.value_of(...) under ValuationRule.
PRICE_TAKING, then combined with plain arithmetic.
...
- Consumer surplus, producer surplus, and total surplus are not
  separate outcome rules, primitives, or helpers, and are not realized
  O. They are read directly from the buyer's and seller's own
  agent.value_of(...) under ValuationRule.PRICE_TAKING, then combined
